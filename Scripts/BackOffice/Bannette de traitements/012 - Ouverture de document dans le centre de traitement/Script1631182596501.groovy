...
for (int i = aydy.toInteger(); ; i++) {
	...
	if (WebUI.getAttribute(findTestObject('Object Repository/Page_Doxtreem - Bannettes vues des traitements/tag', [('index'): i]), 'aria-label', FailureHandling.STOP_ON_FAILURE) == "Gestion facture") {
...
/******************************Trouver la bannette Validation inf à 5000 euros*****************************************************/
WebUI.delay(2)
...
for (int i = ide.toInteger(); ; i++) {
	...
	btnFiltre = "//*[@id=\"inbox-view-" + i + "\"]/md-content/div/div/div"
	if (WebUI.verifyElementText(findTestObject('Object Repository/Page_Doxtreem - Bannettes vues des traitements/titLe', [('index'):aydiInbox]), "Validation inf 5000 euros", FailureHandling.OPTIONAL) ) {
